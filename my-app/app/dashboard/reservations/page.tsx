...
export default function Page () {
    ...
    const fetchReservations = async () => {
      try {
        const response = await fetch(
          "https://intelli-python-backend-56zq.onrender.com/dashboard/reservations/"
        );
  ...
  const handleStatusChange = async (id: number, newStatus: 'accepted' | 'rejected') => {
    try {
      const response = await fetch(`https://intelli-python-backend-56zq.onrender.com/dashboard/reservations/${id}/`, {
        method: 'PATCH',
        headers: {
